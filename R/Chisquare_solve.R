################################################################################
#
# CHISQUARE_SOLVE  Internal function used by CHISQUARE_INV
#
#   PDIFF=chisquare_solve(XGUESS,P,V)  Given XGUESS, a percentile P,
#   and degrees-of-freedom V, return the difference between
#   calculated percentile and P.
#
# Uses GAMMAINC
#
# Written January 1998 by C. Torrence
#-------------------------------------------------------------------------------

chisquare_solve<-function(XGUESS,P,V){

# extra factor of V is necessary because X is Normalized

	# This additional step is necessary in R when V is to large (it decreases V until it pass the 'gammainc' function)
	D=0.99	# This is a relative decrease of V which is by default 0.99 - it can be decreased to speed up calculation process
	suppressWarnings(while(gammainc(V*XGUESS/2,V/2)[3] %in% NA){V=D*V}) 

	PGUESS=as.numeric(gammainc(V*XGUESS/2,V/2)[3])		# incomplete Gamma function
	
	PDIFF=abs(PGUESS-P)					# error in calculated P
	
	TOL=1E-4
	if(PGUESS>=1-TOL){  					# if P is very close to 1 (i.e. a bad guess)
		PDIFF=XGUESS   					# then just assign some big number like XGUESS
	}

	return(PDIFF)
}