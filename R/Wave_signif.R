...
wave_signif<-function(Y,dt,scale1,sigtest,lag1,siglvl,dof,mother,param){

nargin=length(as.list(match.call()))-1	

if(nargin<9){param=-1}
if(nargin<8){mother=-1}
if(nargin<7){dof=-1}
if(nargin<6){siglvl=-1}
if(nargin<5){lag1=-1}
if(nargin<4){sigtest=-1}
if(nargin<3){stop('Must input a vector Y, sampling time DT, and SCALE vector')}
...
if(sigtest==0){									# no smoothing, DOF=dofmin [Sec.4]
	...
	navg=length(avg)
	if(navg==0){
		stop(paste('No valid scales between ',toString(s1),' and ',toString(s2)))
	}
	...
	stop('sigtest must be either 0, 1, or 2')
}

out=list(signif=signif,fft_theor=fft_theor)
}